...
class Media extends Root {
	...
	private function _finalize() {
		...
		if ($cfg_lazy || $cfg_iframe_lazy) {
			$lazy_lib      = '<script data-no-optimize="1">window.lazyLoadOptions = { threshold: ' . apply_filters('lsc_lazyload_threshold', 300) . ' }; ' . File::read(LSCWP_DIR . self::LIB_FILE_IMG_LAZYLOAD) . '</script>';
