...
class Admin_Settings extends Base {
	...
	public function save( $raw_data ) {
		...
		// Method = Auto is now a first-class persisted value, not a save-time
		// sentinel — Object_Cache resolves it to a concrete backend at runtime
		// via auto_detect() and re-resolves on each boot, so the choice keeps
		// tracking the live host environment. No rewriting is done here.
		//
		// Connection-test and Auto-resolution transient busts (plus the
		// Object_Cache singleton reset, so the post-save render reflects the
		// just-saved host/port/method) are owned by Conf::update_confs.
