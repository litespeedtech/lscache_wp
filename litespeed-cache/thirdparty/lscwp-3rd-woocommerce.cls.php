...
class LiteSpeed_Cache_ThirdParty_WooCommerce
{
	...
	public function add_hooks()
	{
		...
		LiteSpeed_Cache_API::hook_control( [ $this, 'set_control' ] ) ;
		LiteSpeed_Cache_API::hook_tag( [ $this, 'set_tag' ] ) ;
		...
		LiteSpeed_Cache_API::hook_get_options( [ $this, 'get_config' ] ) ;
		add_action( 'comment_post', [ $this, 'add_review' ], 10, 3 ) ;

		if ( $this->esi_eanbled ) {
			if ( function_exists( 'is_shop' ) && ! is_shop() ) {
				LiteSpeed_Cache_API::hook_tpl_not_esi( [ $this, 'set_block_template' ] ) ;
				...
				LiteSpeed_Cache_API::hook_tpl_esi( 'storefront-cart-header', [ $this, 'load_cart_header' ] ) ;
				LiteSpeed_Cache_API::hook_tpl_esi( 'widget', [ $this, 'register_post_view' ] ) ;
			}

			if ( function_exists( 'is_product' ) && is_product() ) {
				LiteSpeed_Cache_API::hook_esi_param( 'widget', [ $this, 'add_post_id' ] ) ;
			}
			 ...
			 * @since  1.7.2
			 */
			add_action( 'template_include', [ $this, 'check_if_need_esi' ] ) ;
			LiteSpeed_Cache_API::hook_vary_finalize( [ $this, 'vary_maintain' ] ) ;

		}

		if ( is_admin() ) {
			LiteSpeed_Cache_API::hook_purge_post( [ $this, 'backend_purge' ] ) ;
			add_action( 'delete_term_relationships', [ $this, 'delete_rel' ], 10, 2 ) ;
			LiteSpeed_Cache_API::hook_setting_tab( [ $this, 'add_config' ], 10, 3 ) ;
			LiteSpeed_Cache_API::hook_setting_save( [ $this, 'save_config' ], 10, 2 ) ;
			LiteSpeed_Cache_API::hook_widget_default_options( [ $this, 'wc_widget_default' ], 10, 2 ) ;
		}
	...
	public function check_if_need_esi( $template )
	{
		if (  $this->vary_needed() ) {
			...
			LiteSpeed_Cache_API::hook_tpl_not_esi( [ $this, 'set_swap_header_cart' ] ) ;
		}
	...
	public function set_control($esi_id)
	{
		...
		if ( in_array($uri, ['cart/', 'checkout/', 'my-account/', 'addons/', 'logout/', 'lost-password/', 'product/']) ) {
			LiteSpeed_Cache_API::set_nocache() ;
			return ;
		}
