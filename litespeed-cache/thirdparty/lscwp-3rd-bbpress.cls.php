...
LiteSpeed_API::register('LiteSpeed_Cache_ThirdParty_BBPress') ;
...
class LiteSpeed_Cache_ThirdParty_BBPress
{
	...
	public static function detect()
	{
		if ( function_exists('is_bbpress') ) {
			LiteSpeed_API::hook_purge_post('LiteSpeed_Cache_ThirdParty_BBPress::on_purge') ;
			if ( LiteSpeed_Cache_Router::esi_enabled() ) {// don't consider private cache yet (will do if any feedback)
				LiteSpeed_API::hook_control('LiteSpeed_Cache_ThirdParty_BBPress::set_control') ;
			}
		}
	}
	...
	public static function set_control()
	{
		...
		if ( is_bbpress() && LiteSpeed_Cache_Router::is_logged_in() ) {
			LiteSpeed_API::set_nocache( 'bbpress cant cache loggedin' ) ;
		}
	...
	public static function on_purge($post_id)
	{
		...
		LiteSpeed_API::purge(LiteSpeed_API::TYPE_ARCHIVE_POSTTYPE . bbp_get_forum_post_type()) ;
		$ancestors = get_post_ancestors($post_id) ;

		// If there are ancestors, need to purge them as well.
		if ( ! empty($ancestors) ) {
			foreach ($ancestors as $ancestor) {
				LiteSpeed_API::purge(LiteSpeed_API::TYPE_POST . $ancestor) ;
			}
		}

		global $wp_widget_factory;
		if ( bbp_is_reply($post_id) && ! is_null($wp_widget_factory->widgets['BBP_Replies_Widget']) ) {
			LiteSpeed_API::purge(LiteSpeed_API::TYPE_WIDGET . $wp_widget_factory->widgets['BBP_Replies_Widget']->id) ;
		}
		if (bbp_is_topic($post_id) && ! is_null($wp_widget_factory->widgets['BBP_Topics_Widget']) ) {
			LiteSpeed_API::purge(LiteSpeed_API::TYPE_WIDGET . $wp_widget_factory->widgets['BBP_Topics_Widget']->id) ;
		}
	}
}
