...
class Crawler_Engine
{
	...
	public function refresh_list_size()
	{
		...
		$this->_meta['list_size'] = File::count_lines($this->_sitemap_file) ;
		$this->save_meta() ;
	...
	{
		return File::save( $this->_meta_file . '.reset', time() , true, false, false ) ;
	}
	...
	{
		while ( $urlChunks = File::read($this->_sitemap_file, $this->_meta['last_pos'], self::CHUNKS) ) {// get url list
			// start crawling
	...
	protected function _terminate_running($end_reason)
	{
		if ( $end_reason === true ) { // Current crawler is fully done
			...
			$count_crawlers = Crawler::get_instance()->list_crawlers( true ) ;
			if ( $this->_meta[ 'curr_crawler' ] >= $count_crawlers ) {
				defined( 'LSCWP_LOG' ) && Log::debug( 'Crawler Lib: _terminate_running Touched end, whole crawled. Reload crawler!' ) ;
				$this->_meta[ 'curr_crawler' ] = 0 ;
	...
	private function _get_curl_options()
	{
		...
		// 	defined( 'LSCWP_LOG' ) && Log::debug( 'Crawler Lib: Enabled HTTP2' ) ;
		 ...
		 */
		$hash = Str::rrand( 6 ) ;
		Conf::update_option( self::ITEM_HASH, $hash, 'crawler' ) ;
	...
	public function save_meta()
	{
		...
		$ret = File::save( $this->_meta_file, json_encode( $this->_meta ), false, false, false ) ;
		return $ret ;
	}
	...
	public function read_meta()
	{
		...
		$meta = File::read($this->_meta_file) ;
		...
		if ( $meta && $meta = json_decode( $meta, true ) ) {
			...
			if ( ! isset($meta['file_time']) || $meta['file_time'] < filemtime($this->_sitemap_file) ) {
				defined( 'LSCWP_LOG' ) && Log::debug( 'Crawler Lib: Sitemap timestamp changed, reset crawler' ) ;
	...
	{
		return array(
			'list_size'			=> File::count_lines($this->_sitemap_file),
			'last_update_time'	=> 0,
			'curr_crawler'		=> 0,
