...
$sitemap_time = $__crawler->sitemap_time();

$crawler_list = $__crawler->list_crawlers();

$meta = $__crawler->read_meta();
...
<?php endif; ?>
		...
		<?php
			$ajaxUrl = $__crawler->get_crawler_json_path();
			if ( $ajaxUrl ):
		?>
		...
		<?php $this->form_action( Core::ACTION_BLACKLIST_SAVE ); ?>
			<p>
				<textarea name="<?php echo Crawler::CRWL_BLACKLIST; ?>" rows="10" class="litespeed-textarea"><?php echo $__crawler->get_blacklist(); ?></textarea>
			</p>
		...
		<div class="litespeed-desc">
			<p><?php echo sprintf(__('Current blacklist has %s item(s).', 'litespeed-cache'), $__crawler->count_blacklist()); ?></p>
