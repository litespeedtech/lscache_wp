...
class LiteSpeed_Cache_Admin
{
	...
	private function __construct()
	{
		...
		add_filter( 'plugin_action_links_' . LSCWP_BASENAME, [ $this->display, 'add_plugin_links' ] ) ;
		...
		if ( defined( 'LITESPEED_ON' ) ) {
			...
			foreach ( $purge_all_events as $event ) {
				...
				if ( in_array( $event, [ 'update_option' ] ) ) {
					continue ;
				}
	...
	public function admin_init()
	{
		...
		if ( ! is_network_admin() && $pagenow === 'options.php' ) {
			register_setting(LiteSpeed_Cache_Config::OPTION_NAME, LiteSpeed_Cache_Config::OPTION_NAME, [LiteSpeed_Cache_Admin_Settings::get_instance(), 'validate_plugin_settings']) ;
		}
		...
		if ( LiteSpeed_Cache_Router::esi_enabled() ) {
			add_action( 'in_widget_form', [ $this->display, 'show_widget_edit' ], 100, 3 ) ;
