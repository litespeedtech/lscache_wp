...
class LiteSpeed_Cache_Admin_Rules_Test extends WP_UnitTestCase {
	...
	public function test_path_search()
	{
		...
		$start_path = '';
		$file = '';
		$parameters = [$stop_path, $start_path, $file];
	...
	public function test_file_combine()
	{
		...
		$after = '';
		$path = '';
		$parameters = [$beginning, $haystack, $after, $path];
	...
	public function test_file_save()
	{
		...
		$path = '/root/tests/readme.html';
		$backup = false;
		$parameters = [$content, $cleanup, $path, $backup];
	...
	public function test_set_common_rule()
	{
		...
		$env = '';
		$flag = '';
		$parameters = [$content, &$output, $wrapper, $cond, $match, $env, $flag];
	...
	public function test_set_rewrite_rule()
	{
		...
		$sub = '';
		$env = '';
		$parameters = [$content, &$output, $wrapper, $match, $sub, $env];
	...
	public function test_set_favicon()
	{
		...
		$output = '';
		$errors = '';
		$parameters = [$haystack, $action, &$output, &$errors];
	...
	public function test_set_cache_resource()
	{
		...
		$output = '';
		$errors = '';
		$parameters = [$haystack, $set, &$output, &$errors];
	...
	public function test_parse_ret()
	{
		...
		$haystack = '';
		$errors = '';
		$parameters = [$ret, &$haystack, &$errors];
	...
	public function test_set_subdir_cookie()
	{
		...
		$buf = '';
		$errors = '';
		$parameters = [$diff, &$haystack, &$buf, &$errors];
	...
	public function test_set_on_upgrade()
	{
		...
		$flag = '';
		$content = '';
		$parameters = [$wrapper, $match, $sub, $flag, &$content];
